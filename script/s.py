        ...
        for dosya in dosyalar:
            ...
            if os.path.isfile(dosya_yolu):
                if random.random() < 0.8:  # %80 train, %20 test olarak ayır
                    kopyalanacak_yol = os.path.join(train_klasor_yolu, klasor)
                else:
                    kopyalanacak_yol = os.path.join(test_klasor_yolu, klasor)
                
                # Eğer hedef klasör yoksa oluştur
                if not os.path.exists(kopyalanacak_yol):
                    os.makedirs(kopyalanacak_yol)
                
                # Dosyayı hedef klasöre kopyala
                shutil.copy(dosya_yolu, kopyalanacak_yol)
